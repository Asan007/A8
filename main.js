...
// Unified tap binder: dedupe touchstart & click to avoid double-trigger
let __lastTapTime = 0;
function bindTap(el, handler){
  if(!el) return;
  el.addEventListener('touchstart', (e)=>{
    __lastTapTime = performance.now();
    try{ handler(e); } finally { e.preventDefault(); }
  }, {passive:false});
  el.addEventListener('click', (e)=>{
    if(performance.now() - __lastTapTime < 450) return; // ignore ghost click
    handler(e);
  });
}
...
bindTap(btnShop, ()=>{ toggleShop(); });
bindTap(btnShopClose, ()=>{ closeShop(); });
bindTap(btnUpClose, ()=>{ closeUpgrade(); });
bindTap(elPause, ()=>{ if(!G.shop && !G.showUp && !G.over){ togglePause(); } });
